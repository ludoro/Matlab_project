...
%---------------1 nodo da una parte, 3 dall'altra parte--------------------
            ...
            if(it_is_cut ~= 0)
                ...
                for i = 5:7
                    info_fract(id_f).cut_tet(end).points...
...
%---------------2 nodi da una parte, 2 dall'altra parte--------------------        
            elseif(sum == 0)
                ...
                if(it_is_cut ~= 0)
                    ...
                    info_fract(id_f).cut_tet(end).points(1:2,:) = ...
                        tet(id_tet).P(nodes_together_1);

                    %metto i 2 sotto
                    info_fract(id_f).cut_tet(end).points(3:4,:) = ...
...
%---------------1 nodo su piano, altri discordi----------------------------
            elseif(sum == 3 || sum == 5)
                ...
                if(it_is_cut ~= 0)
                    ...
                    info_fract(id_f).cut_tet(end).points(2,:) = ...
...
%---------------2 nodi sul piano, altri concordi---------------------------               
            elseif(sum == 10 || sum == 6)
                ...
                if(it_is_cut == 1)
                    ...
                    %metto poliedri
                    info_fract(id_f).cut_tet(end).poly_1 = [1,2,3,4];
                    info_fract(id_f).cut_tet(end).poly_2 = [];
...
%---------------2 nodi sul piano, altri discordi---------------------------
            elseif(sum == 8)
                ...
                if(it_is_cut ~= 0)
                    ...
                    %metto punto basso
                    info_fract(id_f).cut_tet(end).points(4,:) = ...
                        tet(id_tet).P(lonely_point_2);
                    ...
                    info_fract(id_f).cut_tet(end).points(5,...
                        6-coord_to_use(id_f,1)-coord_to_use(id_f,2)) = ...
                            third_coord(1);
                    up = 1;
                    middle = [2,3,5];
